...
function capture() {
  ...
  // Mirror the selfie camera so the saved photo matches the preview
...
<template>
  <div class="relative w-full h-full bg-[#080808] overflow-hidden flex items-center justify-center"
    ...
    <div v-if="isInAppBrowser"
      ...
      <div>
        ...
        <p class="font-sans text-white/50 text-sm leading-relaxed">
          Camera doesn't work inside this app's browser.<br>
          Tap <span class="text-[#6B8CAE]">⋯</span> and choose
          <span class="text-[#6B8CAE] font-semibold">{{ isIOS ? 'Open in Safari' : 'Open in Chrome' }}</span>.
        </p>
      </div>
      <label class="cursor-pointer px-5 py-3 rounded-xl border border-white/15 text-white/60 text-sm font-sans hover:text-white/80 hover:border-white/30 transition-colors">
    ...
    <div v-else-if="status === 'denied'"
      ...
      <div class="text-5xl">🔒</div>
      <div>
        <p class="font-racing text-white text-xl tracking-widest mb-2">CAMERA BLOCKED</p>
    ...
    <div v-else-if="status === 'nocam' || status === 'unsupported'"
      ...
      <div class="text-5xl">📁</div>
      <div>
        <p class="font-racing text-white text-xl tracking-widest mb-2">UPLOAD MODE</p>
        <p class="font-sans text-white/50 text-sm">Camera unavailable — pick a photo instead.</p>
      </div>
      <label class="cursor-pointer px-5 py-3 rounded-xl border border-[#6B8CAE]/50 text-[#6B8CAE] text-sm font-sans hover:bg-[#6B8CAE]/10 transition-colors">
        Choose photo
        <input type="file" accept="image/*" capture="environment" class="sr-only" @change="onFileFallback" />
      </label>
    </div>

    <!-- ─── IDLE / STARTING: tap to start (iOS gesture requirement) ─── -->
    ...
    <template v-if="status === 'live'">
      ...
      <button @click="flipCamera"
        class="absolute top-4 right-[60px] w-9 h-9 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center text-white/70 hover:text-white text-lg z-10 transition-colors">
        ⟳
      </button>
    </template>

    <!-- File fallback ref (for programmatic trigger from parent if needed) -->
